function [u_final, u_intermediate] = navier_stokes_solver(u, Force, a, el_fx, el_bx, el_fy, el_by, dx, dt, mu, rho)
    %% Prelimnayr step
    w_pre = u - dt / 2 * skew_symmetric_advection(u, el_fx, el_bx, el_fy, el_by, dx) + dt / (2 * rho) * Force;
    ...
    %% FInal Step
    w_final = u - dt * skew_symmetric_advection(u_intermediate, el_fx, el_bx, el_fy, el_by, dx) + (dt / 2) * (mu / rho) * laplacian_tube(u, el_fx, el_bx, el_fy, el_by, dx) + dt / rho * Force;
