...
function [u_final, u_intermediate, p] = navier_stokes_solver_pIB(u, Force, a, b, el_fx, el_bx, el_fy, el_by, dx, dy, dt, mu, rho)
    %% Prelimnayr step
    ...
    %% Pressure recovery (from precomputed b matrix)
    p_hat = b(:,:,1).*w_final(:,:,1) + b(:,:,2).*w_final(:,:,2);
    p_hat = ifft(p_hat, [], 2);
    p = real(ifft(p_hat, [], 1));
end