...
for m1=0:(Nx-1)
  for m2=0:(Ny-1)
    t=[pi/Nx;pi/Ny].*[m1;m2];
    s=sin(t);
    navst=(1+(dt/2)*(mu/rho)*(4/(h*h))*(s'*s));
    a(m1+1,m2+1,:,:)=a(m1+1,m2+1,:,:)./navst;
%     a(m1+1,m2+1,1,2)=a(m1+1,m2+1,1,2)/navst(1,2);
%     a(m1+1,m2+1,2,1)=a(m1+1,m2+1,2,1)/navst(2,1);
%     a(m1+1,m2+1,2,2)=a(m1+1,m2+1,2,2)/navst(2,1);
  end
end
